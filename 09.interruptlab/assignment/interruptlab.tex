...
\item The user shall be able to build a number.
    ...
    \item Whenever the user presses and then releases the \textbf{right
        ...
        the least-significant digit shall display {\dviiseg 0} (this being the
...
\item The user shall be able to see the number converted to a different number
    base.
    \begin{enumerate}
    \item Whenever the user presses both \textbf{pushbuttons} simultaneously,
        the \textbf{7-segment display module} shall display the number in the
        opposite number base.
        \begin{enumerate}
    ...
    \item The conversion shall take place immediately after the
        \textbf{pushbuttons} are both depressed.
    \item If the \textbf{left switch} is toggled to the left, then exactly 20
        seconds after the \textbf{pushbuttons} are both depressed, the number
    ...
    \item While the converted number is being displayed, the \textbf{external
        ...
        illuminated.
    \item If the \textbf{pushbuttons} are both depressed, then releasing the
        \textbf{right pushbutton} shall not cause the number to be cleared, and
        releasng the \textbf{left pushbtton} shall not cause the number to be
        negated.
    \end{enumerate}
...
\rubricitem{2}{The pushbuttons' interrupt handler determines which button was
    released.}
\rubricitem{2}{The pushbuttons' interrupt handler determines whether both
    buttons are both pressed at the same time.}
...
\rubricitem{4}{The timer interrupts configured such that, when combined with
    ...
    exactly 20~seconds have passed since both buttons were pressed.}
\item[Conversion Timeout]
\rubricitem{2}{The system is able to determine when exactly 7.5~seconds have
    passed since both buttons were pressed at the same time.}
...
\rubricitem{2}{Converts from decimal to hexadecimal when both pushbuttons are
    pressed. (1 point for correctly-displayed positive numbers; 1 point for
    correctly-displayed negative numbers)}
\rubricitem{2}{Converts from hexadecimal to decimal when both pushbuttons are
    pressed. (1 point for correctly-displayed positive numbers; 1 point for
    correctly-displayed negative numbers)}
...
\rubricitem{1}{If both buttons were pressed, then the number being built does
    not change when the buttons are released.}
...
\checkoffitem{Release right left pushbutton. One or both of two things will
    happen:}
    \begin{itemize}
    \item The Serial Monitor indicates that both pushbuttons are UP.
...
\checkoffitem{Place the left switch in the right (hexadecimal) position. Place
    the right switch in the right (7.5~seconds) position.}
\checkoffitem{Press A. {\dviiseg A} may appear on the display module.}
\checkoffitem{Press and hold both pushbuttons. One or more of three things will
    happen:}
    \begin{itemize}
    \item The Serial Monitor indicates that both pushbuttons are DOWN (might
        indicate that one is DOWN and the other is UP and then indicate that
        both are DOWN).
    ...
    \end{itemize}
    \textit{+2 pushbutton ISR determines whether both buttons are pressed at the
        same time} \\
...
\checkoffitem{Release both buttons. The Serial Monitor may briefly indicate that
    ...
    \textit{+1 the number being built does not change when both buttons are
    released}
\checkoffitem{Place the right switch in the left (20~seconds) position.}
\checkoffitem{Press and release both pushbuttons (press and hold if necessary).
    One or more of the previously-mentioned indications of simultaneous button
    presses happen.}
...
\checkoffitem{Place the right switch in the right (7.5~seconds) position.}
...
\checkoffitem{Place the left switch in the left (decimal) position.}
\checkoffitem{Press and release the right pushbutton. The output is {\dviiseg 0}
    on the display module.}
...
\checkoffitem{Press 0. The output is {\dviiseg{8}too big} on the display
    module.} \\
...
\checkoffitem{Enter the value 1234. The output is {\dviiseg\phantom{8888}1234}
    on the display module.}
\checkoffitem{Press and release both pushbuttons. The output is
    {\dviiseg\phantom{88888}4D2} on the display module.} \\
    \textit{+1 converts positive decimal to hexadecimal}
\checkoffitem{Wait 7.5~seconds for the conversion to timeout, and press and
    release the left pushbutton. The output is {\dviiseg\phantom{888}-1234} on
    the display module.}
\checkoffitem{Press and release both pushbuttons. The output is
...
\checkoffitem{Press and release the right pushbutton. The output is {\dviiseg 0}
    on the display module.}
\checkoffitem{Place the left switch in the right (hexadecimal) position.}
...
\checkoffitem{Wait 7.5~seconds for the conversion to timeout, and press and
    release the left pushbutton. The output is {\dviiseg FFFFA949} on
    the display module.}
\checkoffitem{Press and release both pushbuttons. The output is
