...
\lstinputlisting[linerange=54-55, firstnumber=54]{../starter-code/problem5main.c}
...
\subsubsection{Double-Linked List} \label{subsec:doublylinkedlist}

You may see a small performance improvement if you create a circular doubly-linked list instead of a circular singly-linked list.
Being able to traverse the list forward and backward, when combined with examining the alphabetic distance between the current node's word and the new node's word (or the challenge word), will allow you to reduce the worst-case from $n-1$ node traversals to $\frac{n}{2}$ node traversals.
