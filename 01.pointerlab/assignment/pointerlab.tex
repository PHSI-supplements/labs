...
During your lab period, the TAs will \dots
...
\subsection{Pleistocene Petting Zoo Marquee}
...
\subsection{Math Doesn't Work Right \dots Or Does It?}
...
grading. The files marked as \textit{sorted} have all of their words already in
alphabetically sorted order, ignoring capitalization; the files marked as
\textit{unsorted} do not have their words sorted (the words in ``Plants'' and
``Food'' are in a randomly-selected order; the words in ``TheLostWorld'' appear
in the order that they appear in the original \textit{The Lost World}).
...
into the directory that has \textit{pointerlab.c}. If you did not successfully
complete KeyboardLab, we will provide alternate implementations for you.
...
Open \textit{pointerlab.c} in your IDE or editor. You'll see a
\lstinline{struct} with the same fields as our Java example:

\lstinputlisting[linerange=36-41, firstnumber=36]{../starter-code/pointerlab.c}

You'll also see the \function{create_node()} function:

\lstinputlisting[linerange=43-49, firstnumber=43]{../starter-code/pointerlab.c}
...
\lstinputlisting[linerange=51-56, firstnumber=51]{../starter-code/pointerlab.c}
...
\function{insert_after()}, go to the \function{main()} function and un-comment
the call to \function{test_linked_list_functions()}.

\lstinputlisting[linerange=180-181, firstnumber=180]{../starter-code/pointerlab.c}
...
\function{to_lowercase()} function into \textit{pointerlab.c}; we will link to
the function in \textit{problem2.c}).

\lstinputlisting[linerange=68-82, firstnumber=68]{../starter-code/pointerlab.c}
...
\lstinputlisting[linerange=87-108, firstnumber=87]{../starter-code/pointerlab.c}

Add code to \function{insert_word()} and \function{build_list()} to read the
specified file one line at a time.\footnote{See \S7.5 of Kernighan \& Ritchie's
...
\lstinputlisting[linerange=114-130, firstnumber=114]{../starter-code/pointerlab.c}
...
is not present in the list, output ``(word) is not present!'' where ``(word)''
...
present in the list and the number of occurrences is even, output ``(word) has
(number) of occurrences'' where ``(word)'' is the challenge word and
``(number)'' is the number of occurrences. If the number of occurrences is odd
then use the number of occurrences recorded in that word's node to find the
response word as described in the challenge-and-response rules, and output that
word. (Do not copy the \function{is_even()} function into
\textit{pointerlab.c}; we will link to the function in \textit{problem3.c}.)
...
make a backup copy of \textit{pointerlab.c} or to commit it to a private Git
repository.
...
\subsection{Designing the Data Structure and Its Algorithms, part 2: Doubly-Linked List}\label{sec:DoublyLinkedList}
...
\lstinputlisting[linerange=67-72, firstnumber=67]{../starter-code/pointerlab.c}

(If you already implemented Insertion Sort, you probably will not use
\function{insert_before()} except to test that you implemented it correctly;
however, it is still an appropriate function to implement for a doubly-linked
list, and its implementation is part of your grade for this assignment.)
...
occurrences is even, the output is not ``(word) has (number) of occurrences''
but is instead the appropriate response word as described in the challenge-and-
response rules.
...
backup copy of \textit{pointerlab.c} or to commit it to a private Git
repository.

\subsection{Inserting Words, part 1: Insertion Sort}\label{sec:InsertionSort}
...
Merge Sort. Insertion Sort has a particular advantage in that it can be applied
\textit{as the list is built}, making for a much simpler and less error-prone
implementation than a different sort that requires the list to already be built.
...
\textit{pointerlab.c} or to commit it to a private Git repository.
...
arrival, ``This is Lilith Redd from business operations.'' He turns to her and
continues, ``Lil, what's the good word?''
