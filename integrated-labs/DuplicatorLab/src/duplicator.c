...
void duplicate(FILE *source_file, FILE *destination_file) {
    ...
    pthread_create(&source_thread, nullptr, read_original, source_file);
    pthread_create(&destination_thread, nullptr, write_copy, destination_file);
    pthread_join(source_thread, nullptr);
    pthread_join(destination_thread, nullptr);
}
