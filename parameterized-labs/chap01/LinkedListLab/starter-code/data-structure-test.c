...
/**************************************************************************//**
 *
 * @file data-structure-test.c
 *
 * @author Christopher A. Bohn
 *
 * @brief Functions to test the data structures used by the "challenge/response"
 *      application.
 *
 ******************************************************************************/
...
#define NUMBER_OF_LIST_FUNCTIONS (9)
...
void test_list(void) {
    printf("\nTesting word_entry functions...\n");
    char *function_names[NUMBER_OF_LIST_FUNCTIONS + 1] = {
            "Return to main menu",
            "create_list()",
            "delete_list()",
            "reset_iterator()",
    ...
    bool success;
    while (running) {
        for (int i = 0; i < NUMBER_OF_LIST_FUNCTIONS + 1; i++) {
            printf("%d) %s\n", i, function_names[i]);
        }
        printf("Select the function you wish to check: ");
        scanf("%79s", buffer);
        int option = (int) strtol(buffer, NULL, 10);
        printf("\n");
        if (option < 0 || option > NUMBER_OF_LIST_FUNCTIONS) {
            printf("Invalid choice %d. Please select a choice between 0 and %d.\n", option, NUMBER_OF_LIST_FUNCTIONS);
        } else {
            switch (option) {
                case 0:
                    if (errno == EINVAL) {
                ...
                case 3:
                    ...
                    if (list == NULL) {
                        printf("Creating list first.\n");
                        list = create_list();
                    }
                    success = reset_iterator(list);
                    if (success) {
                        printf("Iterator reset.\n");
                    } else {
                        printf("Iterator not reset.\n");
                    }
                    print(list);
                    break;
                case 4:
                    ...
                    success = iterate_forward(list);
                    if (success) {
                        printf("Iterated forward.\n");
                    } else {
                        printf("Iterator not changed.\n");
                    }
                    print(list);
                    break;
                case 5:
                    ...
                    success = iterate_backward(list);
                    if (success) {
                        printf("Iterated backward.\n");
                    } else {
                        printf("Iterator not changed.\n");
                    }
                    print(list);
                ...
                case 6:
                    ...
                    printf("Getting word entry: %s\n", word_entry_to_string(buffer, get_word_entry(list)));
                    break;
                case 7:
                    if (list == NULL) {
                        printf("Creating list first.\n");
