...
/* DETERMINE THE INTEGER PORTION, THE FRACTION, AND THE EXPONENT */
        \end{lstlisting}
    \checkoffitem{Assign the appropriate value to \lstinline{integer}, the implicit integer portion of the significand.}
    \checkoffitem{Assign the appropriate value to \lstinline{fraction}, which is simply the fraction bits as they appear in the \lstinline{number}.}
    \checkoffitem{Assign the appropriate value to \lstinline{exponent}, the two's complement representation of the exponent that you get after removing the bias from the \lstinline{number}'s \texttt{E} field.}
\end{description}
...
\begin{description}
    \checkoffitem{Assign the appropriate value to \lstinline{integer}.}
    \checkoffitem{Assign the appropriate value to \lstinline{fraction}.}
    \begin{itemize}
        \item Hint: if your fraction-related code from \function{ieee754_to_string()} has the fraction bits in the \lstinline{NUMBER_OF_FRACTION_BITS} least-significant bits,
    ...
    \end{itemize}
    \checkoffitem{Assign the appropriate value to \lstinline{exponent}.}
\end{description}
