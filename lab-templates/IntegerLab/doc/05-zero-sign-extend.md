...
(For example, zero-extending `0b1010'1000` from 8 bits to 32 bits gives us `0b0000'0000'0000'0000'0000'0000'1010'1000`.)
...
(For example, sign-extending `0b1010'1000` from 8 bits to 32 bits gives us `0b1111'1111'1111'1111'1111'1111'1010'1000` because the original most significant bit is `1`,
and sign-extending `0010'1000` to 32 bits gives us `0b0000'0000'0000'0000'0000'0000'0010'1000` because the original most significant bit is `0`.)
...
You will zero-extend it to become a 32-bit unsigned value,
according to these rules ($variable_{x..y}$ refers to a range of bits in the `variable`'s bit vector):

| $from\_size$ | |      $result_{31..16}$       |      $result_{15..8}$       |      $result_{7..0}$       |
|:------------:|-|:----------------------------:|:---------------------------:|:--------------------------:|
|  `ONE_BYTE`  | |              0               |              0              | $value_{7..0}$ (unchanged) |
| `TWO_BYTES`  | |              0               | $value_{15..8}$ (unchanged) | $value_{7..0}$ (unchanged) |
...
- [ ] Implement `zero_extend()` to zero-extend the input value from its original size up to 32 bits.
...
That is, the only bits you care about are the lower 8, 16, or 32.
You will zero-extend it to become a 32-bit signed value,
according to these rules ($variable_{x..y}$ refers to a range of bits in the `variable`'s bit vector):

| $from\_size$ | |         $result_{31..16}$          |        $result_{15..8}$        |      $result_{7..0}$       |
|:------------:|-|:----------------------------------:|:------------------------------:|:--------------------------:|
|  `ONE_BYTE`  | |  $value_7$ (duplicated 16 times)   | $value_7$ (duplicated 8 times) | $value_{7..0}$ (unchanged) |
...
```text
Expression to evaluate: extend 0x12345678 8
Bit vector: 0x12345678
Unsigned: initial 8-bit value:         120
	Expected 32-bit value:         120 (0x00000078)	Actual 32-bit value:         120 (0x00000078)
Signed: initial 8-bit value:           120
	Expected 32-bit value:         120 (0x00000078)	Actual 32-bit value:         120 (0x00000078)

Expression to evaluate: extend 0xABCDEF90 ONE_BYTE
Bit vector: 0xabcdef90
Unsigned: initial 8-bit value:         144
	Expected 32-bit value:         144 (0x00000090)	Actual 32-bit value:         144 (0x00000090)
Signed: initial 8-bit value:          -112
	Expected 32-bit value:        -112 (0xffffff90)	Actual 32-bit value:        -112 (0xffffff90)

Expression to evaluate: extend 0xABCDEF98 16
