...
//
// @brief Hand-compiled assembly code for the Caesar Cipher in AddressingLab (A64, Linux).
//
////////////////////////////////////////////////////////////////////////////////
...
//        int reduced_character = *text - 'A';
    sub     w12, w11, #0x41             // w12: reduced_character
...
//        destination[i] = (char) upper[(size_t) sentence[i]];  // eliminate a function call
    ldrb    w11, [x21, x10]             // w11: sentence[i]
    adrp    x12, .LANCHOR0
    add     x12, x12, :lo12:.LANCHOR0   // x12: upper
    ldr     w12, [x12, w11, uxtw 2]     // w12: upper[sentence[i]]
    strb    w12, [x20, x10]
//    }
    add     x10, x10, #1
    cmp     x10, x0
...
//    is_valid = is_valid && (strlen(package->ciphertext) == (size_t) package->sentence_length);
    ldr     x0, [x20, #8]               // x0: package->ciphertext
