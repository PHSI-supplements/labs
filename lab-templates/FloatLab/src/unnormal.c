...
#if defined(__STDC_VERSION__) && __STDC_VERSION__ >= 202311L && !defined(__apple_build_version__)   // prefer C23 type-size modifier
        sprintf(destination, "%c%016w64x.%016w64x_{16} x 2^{%w32d}",
#else                                                                                               // but fall back to C99 type-size modifier if we need to
        sprintf(destination, "%c%016" PRIx64 ".%016" PRIx64 "_{16} x 2^{%" PRIi32 "d}",
#endif
