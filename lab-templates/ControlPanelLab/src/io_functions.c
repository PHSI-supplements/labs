...
/*
 * ControlPanelLab assignment and starter code (c) 2021-25 Christopher A. Bohn
 * ControlPanelLab solution (c) the above-named student(s)
 */
...
static bool key_is_pressed(void) {
    ...
    bool key_is_pressed = !(digitalRead(10) && digitalRead(11) && digitalRead(12) && digitalRead(13));
 ...
 *
 * Prints, the position of the buttons and switches, and any key that is
 * pressed. If both switches are pressed, then the left LED illuminates; if
 * both switches are to the right, then the right LED illuminates.
 *
...
void test_io(void) {
    ...
    if (left_switch_is_in_left_position() != left_switch_position) {
        left_switch_position = !left_switch_position;
        change_is_present = true;
    }
    if (right_switch_is_in_left_position() != right_switch_position) {
    ...
    if (change_is_present) {
        char output[18];
        set_left_led(left_switch_is_in_right_position() && left_button_is_pressed());
        set_right_led(right_switch_is_in_right_position() && right_button_is_pressed());
        ...
        if (show_key_press) {
            ...
            if (key_pressed <= 0x0F) {
                sprintf(output, "%2X%5c%2c%4c%2c",
                        ...
                        left_switch_position ? 'L' : 'R', right_switch_position ? 'L' : 'R');
            }  else {
