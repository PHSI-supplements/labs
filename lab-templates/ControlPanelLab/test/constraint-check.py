"""
constraint-check.py (PlatformIO `pio run` version)
(c) 2021-25 Christopher A. Bohn
...
def after_build(source, target, env):
    lowercase_lab_name: str = os.path.basename(env.subst("$PROJECT_DIR")).lower()
    constraint_file: str = os.path.join('test', f'{lowercase_lab_name}.json')
    exit_status: int = main(['constraint-check.py', f'{constraint_file}'])
    if exit_status == 1:
...
# TODO: figure out how to run the constraint checker before the build (or as a test)
# env.AddPreAction('$BUILD_DIR/src/io_functions.c.o', before_build)     <-- (hardcoding issue aside) this only works before the .o file is created for the very first time, not before it's updated
...
def main(args: List[str]) -> int:
    if len(args) != 2:
        ...
        with open(args[1], 'r') as rules_file:
                ...
                with open(filename, 'r') as source_code_file:
                    ...
                    if len(violations_in_file) == 0:
                        report.append(f'constraint-check.py found no violations'
                                      f' specified by {args[1]} in {filename}.')
                    else:
