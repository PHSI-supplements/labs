...
static uint32_t measure_helper(char *destination, int working_set_size) {
    int stride = 8;
    uint32_t start, stop;
    uint32_t sum = 0;
    ...
    for (long long i = 0; i < number_of_iterations; i++) {
        for (uint8_t volatile *data = cache; data < end_of_data; data += stride) {
            sum += data[i];
        }
    }
...
//           working_set_size, (stop - start));
    sprintf(destination, "%5dB:%12luus", working_set_size, (stop - start));
    return sum;
}
...
//    printf("\nTrying differently-sized working sets to determine cache sizes.\n");
    uint32_t lines_to_print = 0;
    sprintf(destination[lines_to_print++], "working set      time");
    for (int working_set_size = 32; working_set_size < MAX_WORKING_SET_SIZE; working_set_size *= 2) {
        measure_helper(destination[lines_to_print++], working_set_size);
        measure_helper(destination[lines_to_print++], working_set_size + working_set_size / 4); // approximately +1dB
        measure_helper(destination[lines_to_print++], working_set_size + working_set_size / 2); // approximately +2dB
    }
    measure_helper(destination[lines_to_print++], MAX_WORKING_SET_SIZE);
//    printf("\n");
//    return 0;
...
//    printf("\nTrying different strides to determine cache line size.\n");
    uint32_t lines_to_print = 0;
    sprintf(destination[lines_to_print++], "stride           time");
    uint32_t start, stop;
    uint32_t sum = 0;
    int data_size = 1 << 10;   // 1 KB
...
//        printf("Time spent working with data when stride is %d bytes: %lu microseconds\n", stride, (stop - start));
        sprintf(destination[lines_to_print++], "%4dB:%13luus", stride, (stop - start));
    }
//    printf("\n");
//    return sum;
    sprintf(destination[lines_to_print], "%lu", sum);    // throw-away, but forces compiler not to optimize-away the function calls
    return lines_to_print;
}
