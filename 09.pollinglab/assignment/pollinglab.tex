...
%, and Table~\ref{table:DataDirection} shows how to configure the specific pins.
...
\lstinputlisting[linerange=188-192]{../starter-code/PollingLab/cowpi.h}
...
\lstinputlisting[linerange=184-186]{../starter-code/PollingLab/cowpi.h}
...
\lstinputlisting[linerange=42-42]{../starter-code/PollingLab/cowpi.h}
\lstinputlisting[linerange=139-145, breaklines=true,
...
illuminated).
% You can use this function to check whether you wrote
...
you cannot use in this lab.
% After you've satisfied yourself that you correctly
% configured pins \texttt{A4}, \texttt{A5}, \texttt{D8}, \texttt{D9}, and
% \texttt{D12}, you need to remove these functions.
You need to remove these functions.
...
the lookup table.
\lstinputlisting[linerange=26-37]{../starter-code/PollingLab/PollingLab.ino}
...
least one of them holds the value 0.
    ...
    \item Find the \lstinline{if} block starging on line 56 in
        ...
        \lstinputlisting[linerange=56-65]{../starter-code/PollingLab/PollingLab.ino}
...
\lstinputlisting[linerange=68-76]{../starter-code/PollingLab/PollingLab.ino}
...
\lstinputlisting[linerange=194-198]{../starter-code/PollingLab/cowpi.h}
...
\lstinputlisting[linerange=155-156]{../starter-code/PollingLab/cowpi.h}
...
\lstinputlisting[linerange=39-45]{../starter-code/PollingLab/PollingLab.ino}
...
with requirement~\ref{spec:BuildingValue}. Here are some recommendations:
    ...
    \item Keep an array of the bit patterns to be displayed separate from the
        actual value being built. While it won't matter as much in when using
        the hexadecimal number base, constantly re-calculating the BCD
        representation of a value every time a key is pressed in decimal mode
        may become a performance bottleneck since division (and modulo) is not
        implemented in hardware \textit{and} even the (8-bit) hardware-
        implemented arithmetic will require several processor cycles for 32-bit
    ...
    \item When a key is pressed, update both the value being built, the
        ...
        the array.
        \begin{itemize}
        \item Some students in the past recalculated the value being built after
        each keypress by applying a decimal (or hexadecimal) weighted-sum of the
        individual digits. This works but requires seven multiplications and
        seven additions. \textit{Updating} the value being built only requires
...
Rubric:
\begin{description}
\rubricitem{8}{Simple input/output functions correctly.}
...
\rubricitem{2}{When in demonstration  mode, the numeral corresponding to the key pressed is displayed in the least-significant digit on the Display Module, in accordance with requirements~\ref{spec:decimalExplained}, \ref{spec:hexadecimalExplained}, and \ref{spec:demonstrationKeyPress}.}
\rubricitem{1}{When in demonstration mode, the right pushbutton clears the display.}
\rubricitem{2}{Builds a value consistent with requirements~\ref{spec:decimalExplained} and \ref{spec:BuildingValue} when in building mode and using the decimal number base.}
\rubricitem{2}{Builds a value consistent with requirements~\ref{spec:hexadecimalExplained} and \ref{spec:BuildingValue} when in building mode and using the hexadecimal number base.}
\rubricitem{1}{Negates value in the decimal number base when left pushbutton is pressed while in building mode.}
\rubricitem{1}{Negates value in the hexadecimal number base when left pushbutton is pressed while in building mode.}
\rubricitem{1}{Displays correct message when the number being built is too big.}
\rubricitem{1}{When in building mode, prints the number being built (and nothing else) to the Serial Monitor. (This behavior is unspecified when in demonstration mode.)}
...
\penaltyitem{8}{Code associated with conversion mode (other than that covered in the first three penalty items) relies on code that violates the constraints in Section~\ref{sec:Constraints}.}
...
\begin{enumerate}
\item (\phantom{xxx}) Using \function{testSimpleIO()}, show on the Serial Monitor that
    the left and right button presses are detected.
\item (\phantom{xxx}) Using \function{testSimpleIO()}, show on the Serial Monitor that
    the left and right switches' position changes are detected.
\item (\phantom{xxx}) Show that the external LED illuminates when it is supposed to (in a
    ...
    \function{testSimpleIO()}) or by showing the LED illuminate when you
    press a key on the matrix keypad (if you started working on demonstration
    mode).
\item (\phantom{xxx}) Show on the Serial Monitor that key presses on the matrix keypad
    are correctly decoded.
\item (\phantom{xxx}) Show on the display module that values decoded from the matrix
...
\item (\phantom{xxx}) Press the right pushbutton. The display shows \\
    ...
    {\dviiseg\phantom{00000000}}\hspace{2cm}0
\item (\phantom{xxx}) Place the right switch in the left position. The display
    remains unchanged.
\item (\phantom{xxx}) Press 2, then 3. \\
    {\dviiseg \phantom{0000000}2}\hspace{2cm}2 \\
    {\dviiseg \phantom{000000}23}\hspace{2cm}23
\item (\phantom{xxx}) Press B. The display is unchanged.
\item (\phantom{xxx}) Press the left pushbutton, then 1. \\
    {\dviiseg \phantom{00000}-23}\hspace{2cm}-23 \\
    {\dviiseg \phantom{0000}-231}\hspace{2cm}-231
\item (\phantom{xxx}) Press the left pushbutton, then 4. \\
    {\dviiseg \phantom{00000}231}\hspace{2cm}231 \\
    {\dviiseg \phantom{0000}2314}\hspace{2cm}2314
...
\item (\phantom{xxx}) Press the right pushbutton, and place the right switch in
    the right position. \\
    {\dviiseg\phantom{0000000}0}\hspace{2cm}0
\item(\phantom{xxx}) Press \#, then 7. \\
    {\dviiseg \phantom{0000000}E}\hspace{2cm}E \\
    {\dviiseg \phantom{000000}E7}\hspace{2cm}E7
\item (\phantom{xxx}) Press the left pushbutton. \\
    {\dviiseg FFFFFF19}\hspace{2cm}FFFFFF19
\item(\phantom{xxx}) Press 0, then A. \\
    {\dviiseg FFFFF190}\hspace{2cm}FFFFF190 \\
    {\dviiseg FFFF190A}\hspace{2cm}FFFF190A
\item(\phantom{xxx}) Press the left pushbutton, then *. \\
    {\dviiseg \phantom{0000}E6F6}\hspace{2cm}E6F6 \\
    {\dviiseg \phantom{000}E6F6F}\hspace{2cm}E6F6F
\item(\phantom{xxx}) Press B, then C, then D, then 0. \\
    {\dviiseg \phantom{00}E6F6FB}\hspace{2cm}E6F6FB \\
    {\dviiseg \phantom{0}E6F6FBC}\hspace{2cm}E6F6FBC \\
    {\dviiseg           E6F6FBCD}\hspace{2cm}E6F6FBCD \\
    {\dviiseg \phantom{0}too big}\hspace{2cm}too big
\item (\phantom{xxx}) Press the right pushbutton, and place the right switch in
    the left position. \\
    {\dviiseg\phantom{0000000}0}\hspace{2cm}0
\item (\phantom{xxx}) Press 5, then 6, then 7, then 8, then 9, then 0, then 1, then 2, then 3. \\
    {\dviiseg \phantom{0000000}5}\hspace{2cm}5 \\
    {\dviiseg \phantom{000000}56}\hspace{2cm}56 \\
    {\dviiseg \phantom{00000}567}\hspace{2cm}567 \\
    {\dviiseg \phantom{0000}5678}\hspace{2cm}5678 \\
    {\dviiseg \phantom{000}56789}\hspace{2cm}56789 \\
    {\dviiseg \phantom{00}567890}\hspace{2cm}567890 \\
    {\dviiseg \phantom{0}5678901}\hspace{2cm}5678901 \\
    {\dviiseg           56789012}\hspace{2cm}56789012 \\
    {\dviiseg \phantom{0}too big}\hspace{2cm}too big
\item (\phantom{xxx}) Press the right pushbutton. \\
    {\dviiseg\phantom{0000000}0}\hspace{2cm}0
\item (\phantom{xxx}) Press 3, then the left pushbutton, then 4, then 5, then 6, then 7, then 8, then 9, then 0. \\
    {\dviiseg \phantom{0000000}3}\hspace{2cm}3 \\
    {\dviiseg \phantom{000000}-3}\hspace{2cm}-3 \\
    {\dviiseg \phantom{00000}-34}\hspace{2cm}-34 \\
    {\dviiseg \phantom{0000}-345}\hspace{2cm}-345 \\
    {\dviiseg \phantom{000}-3456}\hspace{2cm}-3456 \\
    {\dviiseg \phantom{00}-34567}\hspace{2cm}-34567 \\
    {\dviiseg \phantom{0}-345678}\hspace{2cm}-345678 \\
    {\dviiseg           -3456789}\hspace{2cm}-3456789 \\
    {\dviiseg \phantom{0}too big}\hspace{2cm}too big
\end{enumerate}
